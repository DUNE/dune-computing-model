...
initiated by cosmic $\mu$ outside of the fiducial volume, with $K_L^0$ getting into the LArTPC
and participating in a charge exchange reaction. The relatively low cross-section of this reaction
combined with muon flux estimates lead to the conclusion that the rates for such background events are rather low
(expected value of less than one event per year if extrapolating from~\cite{muon_bkgd_pdk}).
