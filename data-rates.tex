...
after the nominal $t_0$ (trigger) window. The value of 5.4ms is 2.4 times larger than the nominal drift of 2.25,
and it's listed in the table as the ``drifts/readout'' factor. This factor is not calculated precisely but was chosen based
on the experience with the 35t prototype (\ref{sec:35t}). The motivation behind this exta readout it to caputure
...
possible that the readout time will be revised as the final detector design is progressing, however the value of 5.4ms
...
\end{table}
The first row gives the data size corresponding to one DAQ readout cycle (5.4ms, as explained in \ref{sec:fundamental-parameters}).
