...
Numners shown in Table~\ref{tab:zs-volume} for cosmic-$\mu$ and beam-$\nu$
...
for DUNE 
from the discovery point of view \cite{cdr_vol2}. Kaons from free proton decay are expected to have a kinetic
...
at FNAL and protoDUNE at CERN
(see Sec.~\ref{sec:dune-prototypes} for more details), and this activity is largely aligned with the Far Detector simulations.
...
MC event sizes vary depending on the conditions of the simulation, but on average are estimated to be $\sim$7\,MB per event. At the time
of writing, the typical scale of data produced during a single Monte Carlo campaign is 2.5\,TB. In a course of a year, roughly six MC campaigns
are expected to take place, with resulting data volume of $\sim$15\,TB per year.
...
Types and sources of data generated in the course of DUNE Monte Carlo studies are summarized in Table~\ref{tab:mc-data}.
\begin{table}[ht!]
\centering
...
%of the Computing Model. Principal types of data in DUNE will include raw and processed data.
According to definition given in \ref{sec:req-data-definitions}, processed data includes the Monte Carlo data and
also data derived from either raw data or Monte Carlo, such as reconstructed event elements in any stage of
processing.
...
However, an additional consideration must be given to the extremely likely scenario that same data (in any stage of
its transformation) may be processed a few times, for example with different software releases.
