...
neutrinos in the Far Detector, but not to the extent that this becomes a challenge for the modern type of inftrastructure
...
%Full height & 12.0\,m \\ \hline
%Full width & 12\,m \\ \hline
...
flagged by DAQ as candidate should not be discarded immediately (if at all) even if near-time analysis indicated it was
likely a false positive, for the following reasons:
\begin{itemize}
...
This is obviously a very challenging measurement and a vigorous R\&D program will be required to make it a
...
\subsubsection{Summary of the Revised High-Energy Threshold Data Volume}
...
The Far and Near detectors are effectively looking at different sets of events
(since there is no event-level correlation in the respective recorded data), so
there is no reason to complicate logistics and processing of the data by introducing dependencies
across these different datasets and/or merge data in common files. The ND and FD data streams will be treated and processed separately
 and will require different and distinct calibration procedures.
