...
In December of 2015 it was given the official designation as a CERN experiment ``NP04''.
...
As already mentioned in \ref{sec:protodune-data-volume}, due to nominal values of the cosmic ray muon flux and the parameters of the protoDUNE TPC,
there is expected to be on average $\sim$23 cosmic muon track segments per readout window and on top of the actual beam event.
Given that a 4\,GeV Minimum Ionizing Particle (MIP) will produce on average 80\,kB of data, the resulting number of particles in
...
a few~PB of disk storage at Fermilab to ensure optimal data availability and  processing efficiency. 
