...
Both experiments have opted to not use lossy compression schemes (such as zero suppression) when handling their data.
...
%allowing compressing the charge readout data flow by a factor 10.  Simulation indicates that with realistic noise levels NP04 should achieve a reduction in %data size by a factor of 4 from lossless compression. 
...
substantial (see sections \ref{sec:np04_data_rate} and \ref{sec:np02_online_processing}).
For these reasons, capturing data streams generated by the protoDUNE DAQ systems,
buffering of the data, performing fast Data Quality Monitoring (DQM) analysis,
and transporting the data to sites external to CERN for processing (e.g. FNAL, BNL, etc.)
requires substantial resources and adequate planning.
...
As shown in the diagram, EOS serves as the staging area where the data is first transported from the DAQ buffer.
The EOS copy then serves as the source of  the data which gets committed to CASTOR (tape)
and from which it is transmitted to a number of endpoints including the protoDUNE Prompt Processing System (p3s) and
principal data centers such as FNAL. This architecture is motivated by the proven design of data handling systems in the LHC experiments.

EOS is also used by NP04 as the staging area for to DQM processing which takes place in the CERN Tier-0.
...
\subsection{The \pd Timeline Definition}

The different periods in the \pd{s} lifecycle can affect components of the computing and software model. We define these
periods for reference:

\begin{itemize}
\item The Detector Activation period is the the time span during which all systems are coming online but the detector
volume is not yet filled with Liquid Argon. This time period provides an opportunity to test all kinds of connections and
cabling and ascertain the operation of the DAQ as well as data handling and Online Monitoring and Data Quality Monitoring Systems.
\item The Commissioning starts when the detector is filled with Liquid Argon, and the bias voltages are applied after a series of checks.
This period is focused on exercising the systems in preparation for Cosmic Ray and Beam Data Taking. Cosmic Ray data can be used
for overall validation of the system.
\item  Beam Data Taking Operations during the accelerator beam time.
\item  Cosmic Ray data acquisition that occurs during both accelerator beam and non-beam time.
\item Post beam:  taking Cosmic Ray data when the beam is shutdown for the LHC LS2 in late Fall 2018.
\end{itemize}
\noindent 
Offline Processing has defined as periods of:
\begin {itemize} 
...
\section{Data Handling Requirements}
...
The CPU is provided by a local cluster located in the vicinit of the control room.
...
The beam trigger rate during spills will be 100Hz. Cosmic data will be taken during off-spill time l, with a low rate (O(1Hz)).
Taking into account the  latest beam request for 2018,  the NP02 data volume is presented  inTable~\ref{tab:np02_data_rate}.
...
\item Record file metadata including the file status and outcome of file operations
...
\section{Outline of the Data Handling System Design}
...
and extensive monitoring capabilities. For details refarding its design
and capabiliites please see Appendix\,\ref{sec:appendix-fts}.



\subsection{Data provisioning for Data Quality Monitoring}
\label{sec:prompt}

Data Quality Monitoring (previously termed Prompt processing) is performed at CERN \textit{on
a subset} of the raw data to support commissioning and ongoing data quality monitoring.
...
An example of the cleanup process is shown in flow chart of
...
\label{sec:backlog}
As already mentioned in Sec.\ref{sec:daq_interface}, CERN policies require that an experiment is capable
of storing data locally (in the vicinity of the apparatus as opposed to CERN Central Storage) 
for a nominal period of 3 days, to ensure that the experiment can run for a period of time even in case of network
interruptions and storage facility outages.
...
\item{\textbf{p3s}} See the \textit{Prompt Processing System} below
...
\section{F-FTS}
\label{sec:appendix-fts}

\subsection{Overview of F-FTS}
...
\section{The EHN1 Computing Cluster}\label{sec:ehn1_infra}

There are substantial computing resources located in the EHN1 hall which have to be local
to the experiment (e.g. to fulfill the needs of NP02).

\subsection{Hardware Resources}

The Neutrino Platform (NP) computing cluster located in EHN1 in the immediate vicinity of the NP02 detector
consists of about 300 nodes, serving as an integral part of the CERN Neutrino platform R\&D projects.
These nodes are Intel Xeon-based DELL Poweredge 1950 machines  with 16 cores each. A fraction of these machines
have a 2\,TB hard disk that makes them suitable also for storing data. A few machines are reserved for software
builds to ensure high availability of CPU and fast storage.

\subsection{Configuration Management}
Puppet is used as an automated configuration management system for our cluster. It utilizes virtual machines (VM)
which are hosted within the CERN OpenStack Cloud infrastructure. The whole system is already integrated
with the CERN Puppet Orchestration. The Puppet makes it possible to describe machine configurations in a declarative language,
bring machines to a desired state and perform maintenance through automation. The deployment type of Puppet in this cluster was
the agent/master architecture. Puppet was used with Foreman, an open source tool that helps with the management of servers
by an easy interaction with Puppet to automate tasks and application deployment. Foreman provides a robust web interface that
allows us to provision, configure cluster nodes and leverage its External Node Classifier (ENC) and reporting capabilities to
simplify the management of Puppet.

According to the proposed topology of the system we created the following architecture. 
The main challenge we have encountered was that the system will use  both physical and virtual machines, but a good management of the host groups and environments can solve this problem. For the main host group we created a new environment, lbd branch, and there we tested all the features for the cluster.

\begin{figure}[tbh]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/ehn1-puppet.png}
  \caption{EHN1- cluster system architecture.}
  \label{fig:ehn1-puppet}
\end{figure}
...
Each host group and sub-host group is managed through the Puppet manifest. A puppet run occurs each hour in all the nodes. The manifests are stored using the Git version control system.
 The workers sub-host group contains all the physical machines that are part of the clus- ter and that will run all compute-intensive jobs. In a future HTCondor infrastructure, these machines will be the workers of the system. We use the workers2 sub-host group for testing new modules for our system.
