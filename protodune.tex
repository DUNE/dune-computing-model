...
The Neutrino Platform (NP) computing cluster (so-called ``npcmp'') located in EHN1, near the the immediate vicinity of the NP02 detector, consists of about 300 nodes, serving as an integral role to the CERN Neutrino platform R&D projects. It represents an effort to foster fundamental research in the field of Neutrino physics as it provides data processing facility. These nodes are DELL Poweredge 1950 machines each with 16 cores. A part of these machines have a 2 TB hard disk that makes them suitable also for storing data for different jobs. 
...
The main goal is the implementation of a pilot-based mechanism for computational payload deployment. Neutrino Platfrom Computing group worked in configuring and designing the architecture for the cluster of computers that are located in the EHN1 hall, for their future
